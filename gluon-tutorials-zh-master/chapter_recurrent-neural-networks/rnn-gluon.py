...
class RNNModel(gluon.Block):
    ...
    def __init__(self, mode, vocab_size, embed_dim, hidden_dim,
                 num_layers, dropout=.5, **kwargs):
        super(RNNModel, self).__init__(**kwargs)
        with self.name_scope():
            self.drop = nn.Dropout(dropout)
            self.encoder = nn.Embedding(
                vocab_size, embed_dim, weight_initializer=mx.init.Uniform(.1))
            if mode == 'rnn_relu':
                ...
                self.rnn = rnn.RNN(hidden_dim, num_layers,
                                   dropout=dropout, input_size=embed_dim)
            elif mode == 'gru':
                self.rnn = rnn.GRU(hidden_dim, num_layers,
                                   dropout=dropout, input_size=embed_dim)
            else:
...
print('Test loss %.2f, test perlexity %.2f' % (test_L, math.exp(test_L)))
